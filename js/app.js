...
(function () {
  ...
  function renderToolsGrid() {
    ...
    if (!grid) return;
    grid.innerHTML = Object.keys(TOOLS).map(function (id) {
      const t = TOOLS[id];
  ...
  const KPI_SNAPSHOT_KEY = 'nexora_kpi_snapshots';

  function kpiMonthKey() {
    const d = new Date();
    const parts = [d.getFullYear(), String(d.getMonth() + 1).padStart(2, '0')];
    return parts.join('-');
  }

  function kpiPrevMonthKey() {
    const d = new Date();
  ...
  function writeKPISnapshots(snaps) {
    try { localStorage.setItem(KPI_SNAPSHOT_KEY, JSON.stringify(snaps)); } catch (e) { /* ignore */ }
  }
  ...
  function currentKPIValues() {
    ...
    let procVal = 0;
    for (let i = 0; i < history.length; i++) {
      const m = parseFloat(String(history[i].total || '').replace(/[^0-9.]/g, ''));
      if (isFinite(m)) procVal += m;
    }
  ...
  function applyKpiDelta(deltaEl, cur, prev) {
    if (!deltaEl) return;
    let pct = 0, cls = 'kpi-flat', text = '0%';
    if (cur > 0 && prev === 0) {
      pct = 100;
      cls = 'kpi-up';
      text = '+100%';
      ...
      if (pct > 0) { cls = 'kpi-up'; text = fmtPctShort(pct); }
      else if (pct < 0) { cls = 'kpi-down'; text = fmtPctShort(pct); }
      else { cls = 'kpi-flat'; text = '0%'; }
    }
  ...
  function updateKPICards() {
    ...
    const snap = readKPISnapshots();
    const month = kpiMonthKey();
    if (!snap[month] || typeof snap[month] !== 'object') snap[month] = {};
    snap[month].activeEst = cur.activeEst;
    snap[month].dbCount = cur.dbCount;
    snap[month].docCount = cur.docCount;
    snap[month].procVal = cur.procVal;
    writeKPISnapshots(snap);

    const prev = snap[kpiPrevMonthKey()] || {};
    applyKpiDelta($('kpi-est-delta'), cur.activeEst, prev.activeEst || 0);
    applyKpiDelta($('kpi-db-delta'), cur.dbCount, prev.dbCount || 0);
    applyKpiDelta($('kpi-doc-delta'), cur.docCount, prev.docCount || 0);
    applyKpiDelta($('kpi-val-delta'), cur.procVal, prev.procVal || 0);

    if ($('kpi-val-cached')) {
      $('kpi-val-cached').textContent = 'cached: ' + JSON.stringify(prev);
      $('kpi-val-cached').hidden = true;
    }
  }
  ...
  function renderKpis() {
    ...
    $('kpi-val').textContent = fmtCompact(cur.procVal);
    ...
    const setBar = function (id, val, cap, label) {
      const bar = $(id).closest('.health-item').querySelector('.health-fill');
      const strong = $(id);
    ...
    setBar('hl-storage', usedBytes, STORAGE_QUOTA_BYTES, fmtBytes(usedBytes) + ' / 5 MB');
  ...
  function renderActivity() {
    ...
    list.innerHTML = history.slice(0, 12).map(function (h) {
      ...
      const text = h.type === 'db'
        ? '<strong>' + esc(h.toolName) + '</strong> — ' + esc(h.title)
        : '<strong>' + esc(h.title) + '</strong> · ' + esc(h.toolName);
      return '<div class="activity-item">' +
        '<span class="activity-ico">' + activityIconSvg(kind) + '</span>' +
        '<div class="activity-body">' +
          '<div class="activity-text">' + text + '</div>' +
          '<div class="activity-time">' + timeAgo(h.at) + (h.total ? ' · ' + esc(h.total) : '') + '</div>' +
        '</div>' +
      '</div>';
    }).join('');
    if (clearBtn) clearBtn.hidden = false;
  ...
  function erpDateStr() {
    const d = erpState.date ? new Date(erpState.date + 'T00:00:00') : new Date();
  ...
  function generatePrintHTML(data) {
    ...
    function row(label, value, labelW, opts) {
      ...
      return '<div style="display:flex;align-items:flex-start;' + (o.band ? '' : 'min-height:' + minH + 'pt;') +
        ...
        '<span style="flex:1 1 auto;min-width:0;white-space:pre-wrap;word-wrap:break-word;word-break:break-word;overflow-wrap:anywhere;box-sizing:border-box;">' +
    ...
    const leftBand = '<div style="' + bandStyle + '">' +
      row('Date of Invoice', data.invoiceDate, MXPT.labelW, bandLH) + '</div>';
    const leftBox = row("Supplier's TIN", data.supplierTin, MXPT.labelW) +
      ...
      row('Date of Supply', data.supplyDate, MXPT.labelW);
    const rightBand = '<div style="' + bandStyle + '">' +
      row(docTitle === 'TAX INVOICE' ? 'Tax Invoice No' : 'Quotation No', data.docNo, MXPT.labelWR, bandLH) + '</div>';
    const rightBox = row("Purchaser's TIN", data.clientTin, MXPT.labelWR) +
      row("Purchaser's Name", data.clientName, MXPT.labelWR) +
      row('Address', data.clientAddress, MXPT.labelWR, tallRow) +
      ...
      // printed in the phone slot.
      row('Telephone No', data.clientPhone || '', MXPT.labelWR) +
      row('Place of Supply', data.placeOfSupply || 'N/R', MXPT.labelWR);
    ...
    const addl = '<div style="height:' + MXPT.gutter + 'pt;"></div>' +
      '<div style="' + bandStyle + '">' +
      row('Additional Information', addlInfo, MXPT.labelWA, bandLH) + '</div>';
    ...
    const bodyRow = function (item, index) {
      return '<tr>' +
        '<td style="' + ITEM_TD + 'text-align:center;">' + (index + 1) + '</td>' +
        '<td style="' + ITEM_TD_DESC + '">' + (item.description || '') + '</td>' +
        '<td style="' + ITEM_TD + 'text-align:center;">' + (item.unit || '') + '</td>' +
        '<td style="' + ITEM_TD + 'text-align:center;">' + (item.qty === '' || item.qty === null || item.qty === undefined ? '0' : item.qty) + '</td>' +
        '<td style="' + ITEM_TD + 'text-align:right;">' + mx2(item.rate) + '</td>' +
        '<td style="' + ITEM_TD + 'text-align:right;">' + mx2(item.amount) + '</td>' +
        '</tr>';
    };
    // label cells span the Description+Unit+Qty columns; the No and Rate
    // cells stay empty, exactly as in the master.
    const totalRow = function (label, value, boldLabel, boldValue) {
      const td = TOTAL_TD;
      return '<tr>' +
        '<td style="' + td + '"></td>' +
        '<td colspan="3" style="' + td + 'text-align:center;' + (boldLabel ? 'font-weight:700;' : '') + '">' + label + '</td>' +
        '<td style="' + td + '"></td>' +
        '<td style="' + td + 'text-align:right;' + (boldValue ? 'font-weight:700;' : '') + '">' + mx2(value) + '</td>' +
        '</tr>';
    };
    let totals = totalRow('Sub Total', data.subTotal, true, false);
    if (Number(data.discount) > 0) totals += totalRow('Discount', -Math.abs(Number(data.discount)), true, false);
  ...
  /* ── Live editable document preview ───────────────────────────
     ...
     export step reads this DOM verbatim instead of re-running the
     template, so text the user retypes straight into the page survives
     into the PDF. `pdfPreviewEdited` pins the preview — once hand-edited,
     form changes stop regenerating it (they would silently discard the
     correction) until the user presses "Rebuild from form". */
  let pdfPreviewHtml = '';      // last template output
  let pdfPreviewEdited = false; // true once the sheet has been hand-edited
  let pdfPreviewTimer = null;
  // PDF editor toolbar state (declared here so the painters below can reset
  // it without worrying about declaration order).
  let pdfSelected = null;       // element the toolbar is bound to
  let pdfMoveMode = false;      // drag whole sections instead of typing
  let pdfDrag = null;           // in-flight drag
  ...
  function paintPdfPreview() {
    ...
    pdfPreviewEdited = false;
    pdfPreviewState('Live');
    pdfTagBlocks();          // drop stale drag markers / re-tag when moving
    pdfSyncToolbar(null);
  }

  // Re-render the sheet from the form. Debounced, and skipped while the
  // preview is pinned. `force` unpins (the Rebuild button, resets).
  function schedulePdfPreview(force) {
    if (pdfPreviewEdited && !force) return;
  ...
  /* The print payload: the live sheet, with every editor-only attribute
     removed so no contenteditable/spellcheck state (and no focus ring or
     hover tint) can reach the printed page. */
  function pdfPreviewPayload() {
    const host = $('pdf-preview-container');
    if (!host) return '';
    const clone = host.cloneNode(true);
    // Editor-only state: contenteditable / spellcheck (no focus ring), the
    // selection ring, the drag markers and the preview zoom. None of it may
    // reach the printed page — the user's own inline formatting stays.
    const marked = clone.querySelectorAll('[contenteditable], .pdf-selected, [data-pdf-block]');
    for (let i = 0; i < marked.length; i++) {
      marked[i].removeAttribute('contenteditable');
      marked[i].removeAttribute('spellcheck');
      marked[i].removeAttribute('data-pdf-block');
      marked[i].classList.remove('pdf-selected');
    }
    clone.removeAttribute('contenteditable');
    clone.removeAttribute('spellcheck');
    clone.removeAttribute('aria-label');
    clone.removeAttribute('data-pdf-block');
    clone.removeAttribute('data-pdf-scale');
    clone.classList.remove('pdf-selected', 'pdf-move-mode');
    // Drop ONLY the preview zoom from the host's inline style — any text
    // formatting applied to the sheet root itself must still print.
    if (clone.style) {
      if (clone.style.transform) clone.style.transform = '';
      if (clone.style.transformOrigin) clone.style.transformOrigin = '';
  ...
  /* ── PDF editor toolbar ───────────────────────────────────────
     Every control acts on the element the user clicked, which is outlined
     in blue and named in the toolbar's readout. Formatting is written as
     inline CSS (the template styles colour/size/family per node, so
     inheritance alone would show nothing) and therefore travels with the
     printed payload like any other text edit. */
  function pdfNoteEdit() {
    pdfPreviewEdited = true;
    pdfPreviewState('Edited \u00b7 pinned');
  }

  function pdfRgbToHex(v) {
    const s = String(v || '');
    if (/^#[0-9a-f]{6}$/i.test(s)) return s;
    const m = s.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/i);
    if (!m) return '';
    const h = function (n) { return ('0' + Number(n).toString(16)).slice(-2); };
    return '#' + h(m[1]) + h(m[2]) + h(m[3]);
  }

  function pdfTargetLabel(el) {
    if (!el) return 'Nothing selected';
    const text = (el.textContent || '').trim().replace(/\s+/g, ' ');
    // Never label the element with the editor's own bookkeeping classes.
    const cls = String(el.className || '').split(' ').filter(function (c) {
      return c && c.indexOf('pdf-') !== 0;
    })[0];
    const name = el.tagName.toLowerCase() + (cls ? '.' + cls : '');
    if (!text) return name;
    return name + ' \u2014 ' + (text.length > 38 ? text.slice(0, 38) + '\u2026' : text);
  }

  // Push the selected element's current look back into the controls.
  function pdfSyncToolbar(el) {
    const label = $('pdf-tb-target');
    if (label) label.textContent = pdfTargetLabel(el);
    if (!el) return;
    // A wrapper carries no text style of its own, so read the look from its
    // first text-bearing descendant — otherwise the controls would snap back
    // to the page defaults right after you formatted the selection.
    let probe = el;
    let ownText = false;
    for (let i = 0; i < el.childNodes.length; i++) {
      const n = el.childNodes[i];
      if (n.nodeType === 3 && n.nodeValue.trim() !== '') ownText = true;
    }
    if (!ownText) {
      const t = pdfTextNodes(el);
      if (t.length && t[0] !== el) probe = t[0];
    }
    const cs = window.getComputedStyle(probe);
    const font = $('pdf-tb-font');
    if (font) {
      const first = function (v) { return String(v || '').split(',')[0].replace(/['"]/g, '').trim().toLowerCase(); };
      const want = first(cs.fontFamily);
      for (let i = 0; i < font.options.length; i++) {
        if (first(font.options[i].value) === want) { font.value = font.options[i].value; break; }
      }
    }
    const size = $('pdf-tb-size');
    if (size) size.value = String(Math.round(parseFloat(cs.fontSize) * 0.75 * 10) / 10); // px -> pt
    const color = $('pdf-tb-color');
    const hex = pdfRgbToHex(cs.color);
    if (color && hex) color.value = hex;
    const lh = $('pdf-tb-lh');
    if (lh) {
      const ratio = parseFloat(cs.lineHeight) / parseFloat(cs.fontSize);
      const opts = ['1.2', '1.4', '1.6'];
      for (let i = 0; i < opts.length; i++) {
        if (Math.abs(parseFloat(opts[i]) - ratio) < 0.08) { lh.value = opts[i]; break; }
      }
    }
  }

  function pdfSelect(el) {
    const host = $('pdf-preview-container');
    if (pdfSelected && pdfSelected !== el && pdfSelected.classList) pdfSelected.classList.remove('pdf-selected');
    pdfSelected = (el && el !== host && host && host.contains(el)) ? el : null;
    if (pdfSelected) pdfSelected.classList.add('pdf-selected');
    pdfSyncToolbar(pdfSelected);
  }

  /* Inline colour/size/family are set per node by the template, so a style
     on a wrapper would change nothing visually: collect the descendants that
     actually hold text and style those. */
  function pdfTextNodes(el) {
    const out = [];
    (function walk(node) {
      let hasText = false;
      ...
      if (hasText) out.push(node);
      for (let i = 0; i < node.children.length; i++) walk(node.children[i]);
    })(el);
    return out.length ? out : [el];
  }

  function pdfApplyStyle(prop, value) {
    const host = $('pdf-preview-container');
    if (!host) return;
    // With no selection the sheet ROOT is the target (not the container):
    // the container's own inline style is stripped from the print payload.
    const sheetRoot = host.querySelector('.mx-doc > div') || host.firstElementChild || host;
    const nodes = pdfSelected ? pdfTextNodes(pdfSelected) : [sheetRoot];
    for (let i = 0; i < nodes.length; i++) nodes[i].style[prop] = value;
    pdfNoteEdit();
  }

  function pdfSetScale(raw) {
    const host = $('pdf-preview-container');
    const s = Math.min(1.2, Math.max(0.8, Number(raw) || 1));
    if (host) {
      // transform does not change layout size, so the canvas is resized by
      // hand or the shell would scroll to the unscaled extent.
      host.style.transformOrigin = 'top left';
      host.style.transform = (s === 1) ? '' : 'scale(' + s + ')';
      host.setAttribute('data-pdf-scale', String(s));
      const canvas = host.parentElement;
      if (canvas) {
        canvas.style.width = Math.round(816 * s) + 'px';
        canvas.style.height = Math.round((host.scrollHeight || 1056) * s) + 'px';
      }
    }
    const out = $('pdf-tb-scale-out');
    if (out) out.textContent = s.toFixed(2) + 'x';
  }

  /* Move mode: tag the page's top-level sections so they can be dragged as
     whole units, and suspend text editing so the pointer cannot select text
     mid-drag. Offsets use margins (flow, not absolute), so sections nudge
     past each other without overlapping. */
  function pdfTagBlocks() {
    const host = $('pdf-preview-container');
    if (!host) return;
    const stale = host.querySelectorAll('[data-pdf-block]');
    for (let i = 0; i < stale.length; i++) stale[i].removeAttribute('data-pdf-block');
    if (!pdfMoveMode) return;
    const root = host.querySelector('.mx-doc > div') || host.firstElementChild;
    if (!root) return;
    const blocks = [];
    if (root.children[0]) blocks.push(root.children[0]);   // letterhead band
    const frame = root.children[1];
    if (frame) {
      for (let i = 0; i < frame.children.length; i++) blocks.push(frame.children[i]);
    }
    for (let i = 0; i < blocks.length; i++) blocks[i].setAttribute('data-pdf-block', String(i + 1));
  }

  function pdfSetMoveMode(on) {
    const host = $('pdf-preview-container');
    const btn = $('pdf-tb-move');
    pdfMoveMode = !!on;
    pdfDrag = null;
    if (host) {
      host.classList.toggle('pdf-move-mode', pdfMoveMode);
      host.setAttribute('contenteditable', pdfMoveMode ? 'false' : 'true');
    }
    if (btn) btn.setAttribute('aria-pressed', pdfMoveMode ? 'true' : 'false');
    pdfTagBlocks();
  }

  function pdfDragStart(e) {
    if (!pdfMoveMode || !e.target || !e.target.closest) return;
    const block = e.target.closest('[data-pdf-block]');
    if (!block) return;
    ...
    try { block.setPointerCapture(e.pointerId); } catch (err) { /* ignore */ }
    e.preventDefault();
  }

  function pdfDragMove(e) {
    if (!pdfDrag) return;
    // 1 screen px = 0.75pt at 96dpi, divided by the preview zoom.
    const host = $('pdf-preview-container');
    const s = (host && parseFloat(host.getAttribute('data-pdf-scale'))) || 1;
    const dx = (e.clientX - pdfDrag.x) / s;
    const dy = (e.clientY - pdfDrag.y) / s;
    pdfDrag.el.style.marginLeft = (pdfDrag.ml + dx * 0.75).toFixed(2) + 'pt';
    pdfDrag.el.style.marginTop = (pdfDrag.mt + dy * 0.75).toFixed(2) + 'pt';
    pdfNoteEdit();
    e.preventDefault();
  }

  function pdfDragEnd() { pdfDrag = null; }

  /* Reset Layout: put the untouched master template back and return every
     toolbar control to its default (selection, zoom, drag markers, move
     mode). The template is re-painted, so the preview also un-pins. */
  function pdfResetLayout() {
    pdfDrag = null;
    pdfSelect(null);
    pdfSetMoveMode(false);
    const scale = $('pdf-tb-scale');
    ...
    (function wirePdfPreview() {
      ...
      if (host) {
        host.addEventListener('input', function () {
          if (pdfPreviewEdited) return;
          pdfNoteEdit();
        });
        // Click-to-select: whatever was clicked gets the blue ring and the
        // toolbar binds to it.
        host.addEventListener('click', function (e) {
          const el = e.target;
          if (!el || el === host || !host.contains(el)) { pdfSelect(null); return; }
          pdfSelect(el);
        });
        host.addEventListener('pointerdown', pdfDragStart);
        host.addEventListener('pointermove', pdfDragMove);
        host.addEventListener('pointerup', pdfDragEnd);
        host.addEventListener('pointercancel', pdfDragEnd);
      }
      ...
      /* ── Toolbar controls ─────────────────────────────────────── */
      const fontSel = $('pdf-tb-font');
      if (fontSel) fontSel.addEventListener('change', function () { pdfApplyStyle('fontFamily', this.value); });

      const sizeBox = $('pdf-tb-size');
      const applySize = function (pt) {
        const n = Math.min(48, Math.max(4, Number(pt)));
        if (!isFinite(n)) return;
        if (sizeBox) sizeBox.value = String(Math.round(n * 10) / 10);
        pdfApplyStyle('fontSize', (Math.round(n * 10) / 10) + 'pt');
      };
      const sizeUp = $('pdf-tb-size-up');
      if (sizeUp) sizeUp.addEventListener('click', function () { applySize((Number(sizeBox && sizeBox.value) || 9.8) + 0.5); });
      const sizeDown = $('pdf-tb-size-down');
      if (sizeDown) sizeDown.addEventListener('click', function () { applySize((Number(sizeBox && sizeBox.value) || 9.8) - 0.5); });
      if (sizeBox) sizeBox.addEventListener('change', function () { applySize(this.value); });

      const colorBox = $('pdf-tb-color');
      if (colorBox) colorBox.addEventListener('input', function () { pdfApplyStyle('color', this.value); });

      const lhSel = $('pdf-tb-lh');
      if (lhSel) lhSel.addEventListener('change', function () { pdfApplyStyle('lineHeight', this.value); });

      const scale = $('pdf-tb-scale');
      if (scale) scale.addEventListener('input', function () { pdfSetScale(this.value); });

      const moveBtn = $('pdf-tb-move');
      if (moveBtn) moveBtn.addEventListener('click', function () { pdfSetMoveMode(!pdfMoveMode); });

      const resetBtn = $('pdf-tb-reset');
      if (resetBtn) resetBtn.addEventListener('click', pdfResetLayout);

      // Keep the eyebrow "Edited" hint honest for keyboard edits too.
      if (host) host.addEventListener('keyup', function () { pdfNoteEdit(); });
  ...
  renderUtilityPreview();    renderToolsGrid();
    ...
    updateKPICards();
    wireKPILive();
    showView('erp'); // the Master ERP Engine is the default view
    renderAll();
  renderQr();
  renderBoq();
  renderInv();
  ...
  window.addEventListener('keydown', function (e) {
    if (e.key === 'Escape' && !document.getElementById('history-view').hidden) showView('erp');
  });
  if (state.project) {
